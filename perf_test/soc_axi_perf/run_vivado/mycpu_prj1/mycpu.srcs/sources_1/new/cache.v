...
module cache(
    ...
    assign  cache_ram_data_rdata   = (s==4'b1110 & ~ram_data_wr & word_==2'b11)? w_data_:
                                     hit_way0? 
...
/********************  AXI读请求  ********************/
   ...
   // 发起AXI请求条件：cpu有访存请求 且 访存缺失 且 此位（不脏 或 脏已写入vcm），即使有效 且vcmCache未命中
   assign cache_data_req     =      ram_data_req & miss & ~vcm_ram_data_data_ok & ( (old_way==2'b00 & ~dirty_way0[idx_]) |
        ...
        if(!rst || !ram_data_req) begin
            ...
            case(s)
                4'b0000: begin
                    ...
                    // 缺失脏
                    else if( miss & ~addDirtyReq & ~vcm_ram_data_data_ok) begin
                        s           <=  4'b0001;
                        wen_        <=  16'b0;
                        ofst_       <=  2'b00;
                        wHcode_     <=  2'b11;
