...
//在与data竞争中，写操作的优先级最高
assign vcm_addr_ok = vcm_req && !isWriteRequring;    //在空闲下，首选vcm进行写操作
...
always@(posedge clk) begin
    if(!rst)
        ...
        data_handed <= 1'b1;             
    else if(final_Write_responding)   
        data_handed <= 1'b0;
    else
...
always@(posedge clk) begin
    if(~rst) begin
        ...
        if (~wvalid & (vcm_addr_ok | (data_addr_ok & data_wr)) & ~data_handed) begin
            wvalid <= 1'b1;
        end
        else begin
            if(noCache & wready) begin
                wvalid <= 1'b0;
            end
            else if(~noCache & wlast & wready) begin
                wvalid <= 1'b0;   
            end
            else begin
                wvalid <= wvalid;
