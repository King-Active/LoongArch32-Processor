...
always@(posedge clk)begin
    if(rst) s <= 1'b0;
    else if(s == 1'b0 && (signed_div || unsigned_div)) s <= 1'b1;
    else if(s == 1'b1 && (div_valid_sign || div_valid_unsign)) s <= 1'b0;
    else s <= s;
end
assign sign_div_hide = signed_div & (s == 1'b0);
assign unsigned_div_hide = unsigned_div & (s == 1'b0);
