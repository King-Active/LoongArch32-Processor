module StoreTable(
    ...
    //storeFIFO
    input wire [4:0] out_index,   //发射项在table的索引
    input wire       out_ok,      //发射成功
    output wire [31:0] out_addr,  //发射地址
    output wire [31:0] out_data   //发射数据
    );
    
    localparam indexBits = 5;    //索引长度
    localparam stSize    = 32;   //缓存大小（单位：字）
    
    reg stValid   [stSize-1:0];
    reg stInFIFO  [stSize-1:0];
    reg [31:0] stData  [stSize-1:0];      
    reg [24:0] stTag [stSize-1:0];      //以[6:2]为字索引； 以[31:7]为标记
    ...
    assign st_rData = stData[st_addr[6:2]];
    assign buffer_hit  = ((stTag[st_addr[6:2]])==st_addr[31:7]) && stValid[st_addr[6:2]];
    assign canEnter = instr_nocache ? 1'b0 :  //外设不入
                        //写全字 -> (空 | 命中位置 | 不在队列) 可入
                        (w_hcode==4'b1111) ? (~stValid[st_addr[6:2]] || ((stTag[st_addr[6:2]])==st_addr[31:7]) || ~stInFIFO[st_addr[6:2]]) :  
                        //写半字/字节 -> 命中可入
                         (stValid[st_addr[6:2]] && ((stTag[st_addr[6:2]])==st_addr[31:7]));
...
// 时序逻辑 - 出队列；新增表元素 ； 替换表元素
    always@(posedge clk)begin
        ...
        //指令访问 & 可入-> inqueue     cache空闲 & data_ok -> outFIFO
            ...
            stValid [st_addr[6:2]] <= (isStore & canEnter) ? 1'b1 : stValid [st_addr[6:2]];
            stInFIFO [st_addr[6:2]] <= (isStore & canEnter) ? 1'b1 : stInFIFO [st_addr[6:2]];
            stTag [st_addr[6:2]] <= (isStore & canEnter) ? st_addr[31:7] : stTag [st_addr[6:2]];
            stData [st_addr[6:2]] <= (isStore & canEnter) ? ((st_wData_silled & total_hcode)|(stData [st_addr[6:2]] & (~total_hcode))) : stData [st_addr[6:2]];
        end
    end
